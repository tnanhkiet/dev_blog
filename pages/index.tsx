...
const Home: NextPage = ({
  ...
  return (
    <main className="layout">
      ...
      <section className="fex flex-col items-center text-[1.15rem] mt-12">
        <div className="flex gap-3 mb-12 justify-center">
          {tags.map((tag: string, index: number) => {
            return (
              <button
                key={index}
                className={`bg-sky-600 px-2 mt-2 font-semibold rounded-xl text-zinc-800 hover:bg-sky-400 transition-all duration-300 ${
                  selectedIndex.includes(index) ? 'label-selected' : ''
                }`}
